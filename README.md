...
| Метрика           | Бейзлайн (CNN без подсказки) | Целевой ориентир | Достигнуто на val |
| ----------------- | ---------------------------- | ---------------- | ----------------- |
| `val/acc_no_hint` | ≥ 0.97                       | ≥ 0.99           | **0.988**         |
| `val/acc_hint_AH` | ≥ 0.97                       | ≥ 0.995          | **0.9957**        |

Числа в колонке «Достигнуто» — лучший чекпойнт текущего обучения (см.
[plots/](plots/)). Модель уже применялась на финале ВсОШ по астрономии
(см. врезку выше).
...
- датасет уже собран из сырого NIST: фильтр до букв A–Z, генерация
  синтетики для `<empty>` / `<junk>`, сплит train/val. Раздаётся в готовом
  виде через DVC-remote на DAGsHub (`mira-download-data` под капотом
  делает `dvc pull`) — пересобирать его проверяющему не нужно.
...
через публичный DVC-remote проекта на DAGsHub.
...
тех же синтетических композициях NIST + рамка. Служит нижней границей
качества.
...
- **PyTorch Lightning** (`L.Trainer`, `L.LightningModule`, `L.LightningDataModule`,
  колбэки `ModelCheckpoint`, `LearningRateMonitor`, `EarlyStopping`)
- **Hydra** для конфигов и точки входа
- **MLflow** для логирования метрик и гиперпараметров
- **DVC** (публичный remote на **DAGsHub** для дистрибуции данных и весов)
...
- Обучение: фактически велось на **Apple MacBook Pro M1 Max** (MPS), при
  батче 256 — около 14 мин/эпоху. Дефолтный конфиг — 50 эпох (один косинусный
  цикл `scheduler_t_max=50`) + `EarlyStopping(val/loss, patience=10)`,
  на практике останавливается на 45–50 эпохах, итого ~11–12 ч. Запуск
  без изменений переносится на одиночный GPU класса T4.
...
# 3. данные и веса — публичный DVC-remote на DAGsHub, авторизация не нужна
...
В `.dvc/config` прописан публичный DVC-remote на **DAGsHub**. Скачивание
автоматически встроено в `mira-train` и `mira-infer` через
`download_data()`, которая под капотом зовёт `dvc pull` на нужные
`.dvc`-указатели и проверяет маркер, чтобы не качать повторно. Поэтому
проверяющий запускает `mira-train` / `mira-infer` без каких-либо
предварительных авторизаций — DVC сам сходит на DAGsHub и принесёт
артефакты.
...
│   └── utils/
│       ├── download.py             # wrapper над `dvc pull` по таргетам
